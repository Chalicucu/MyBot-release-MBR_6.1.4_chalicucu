...
Func ParseAttackCSV($debug = False)
	...
	If FileExists($dirAttacksCSV & "\" & $filename & ".csv") Then
					...
					Case "DROP"
						...
						If $value4 = "REMAIN" Then 		;chalicucu drop remain troops
							...
							IF PrepareAttack($iMatchMode, True) > 0 Then
								...
								For $ii = $eBarb To $eLava; lauch all remaining troops
									LauchTroop($ii, 1, 0, 1)
		...
		;chalicucu , from mikemikemikecoc
		; SetLog("Dropping left over troops", $COLOR_BLUE)
		; For $x = 0 To 1
			; IF PrepareAttack($iMatchMode, True) > 0 Then
				; For $i = $eLava To $eBarb Step -1; lauch all remaining troops [chalicucu change from big to small]
					; LauchTroop($i, 1, 0, 1)
					CheckHeroesHealth()
